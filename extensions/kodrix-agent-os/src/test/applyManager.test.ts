...
suite('applyManager', () => {
	...
	suite('diffLines', () => {
		test('空文本 → 空文本 返回空数组', () => {
			const result: DiffLine[] = applyManager.diffLines('', '');
			assert.strictEqual(result.length, 0);
		});

		test('空 → 非空 全新增', () => {
			const result: DiffLine[] = applyManager.diffLines('', 'line1\nline2');
		...
		test('完全相同 全 same', () => {
			const result: DiffLine[] = applyManager.diffLines('a\nb\nc', 'a\nb\nc');
		...
		test('大文件降级：超过 DIFF_DP_CELL_LIMIT 时全删+全增', () => {
			...
			const result: DiffLine[] = applyManager.diffLines(oldLines, newLines);
	...
	suite('renderUnifiedDiff', () => {
		...
		test('相同内容 diff 仅有空格前缀行', () => {
			...
			assert.ok(bodyLines.every((l: string) => l.startsWith(' ')));
		});
	});
