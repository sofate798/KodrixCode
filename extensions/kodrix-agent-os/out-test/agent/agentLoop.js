...
exports.DEFAULT_TOOLS = exports.AGENT_LOOP_SYSTEM_PROMPT = void 0;
exports.resolveInWorkspace = resolveInWorkspace;
exports.parseToolCalls = parseToolCalls;
exports.stripToolCalls = stripToolCalls;
exports.runAgentLoop = runAgentLoop;
exports.buildHistoryContext = buildHistoryContext;
exports.renderTraceMarkdown = renderTraceMarkdown;
exports.registerAgentLoop = registerAgentLoop;
...
// ── 系统提示（工具协议） ─────────────────────────────────────────
exports.AGENT_LOOP_SYSTEM_PROMPT = [
...
function resolveInWorkspace(relPath, workspace) {
...
const readFileTool = {
    ...
    async execute(args, session) {
        ...
        if (!p)
            return { ok: false, output: '路径越界：仅允许工作区内文件' };
        if (!fs.existsSync(p) || fs.statSync(p).isDirectory())
            return { ok: false, output: `文件不存在或为目录：${args.path}` };
        try {
...
const editFileTool = {
    ...
    async execute(args, session) {
        ...
        if (!oldText)
            return { ok: false, output: 'old 不能为空' };
        if (!fs.existsSync(p))
            return { ok: false, output: `文件不存在：${args.path}` };
        try {
            const content = fs.readFileSync(p, 'utf-8');
            const count = content.split(oldText).length - 1;
            if (count === 0)
                return { ok: false, output: `未找到匹配文本（${args.path}）。请先 read_file 核对内容。` };
            if (count > 1)
                return { ok: false, output: `匹配 ${count} 处，old 必须唯一。请扩大上下文。` };
...
const listDirTool = {
    ...
    async execute(args, session) {
        ...
        if (!fs.existsSync(p) || !fs.statSync(p).isDirectory())
            return { ok: false, output: `目录不存在：${args.path ?? '.'}` };
        try {
...
const searchTool = {
    ...
    async execute(args, session) {
        ...
        if (!query)
            return { ok: false, output: 'query 不能为空' };
        ...
        const matchGlob = (fileName, relPosix) => {
            if (!glob)
                return true;
            const extFromStar = (p) => p.startsWith('*') && !p.includes('/') ? p.slice(1) : undefined;
            if (glob.startsWith('**/')) {
...
const codebaseSearchTool = {
    ...
    async execute(args, session) {
        ...
        if (!query)
            return { ok: false, output: 'query 不能为空' };
        try {
            const { searchSymbolsAsync, searchFilesAsync } = require('../codebase/semanticIndex');
            const { getProjectIndex, ensureProjectIndex } = require('../codebase/projectIndexer');
...
const runCommandTool = {
    ...
    async execute(args, session) {
        if (!session.allowCommands)
            return { ok: false, output: '命令执行已禁用（kodrix.agentLoop.allowCommands=false）' };
        const command = String(args.command ?? '');
        if (!command.trim())
            return { ok: false, output: 'command 不能为空' };
        try {
            const res = await (0, terminalAi_1.runCommandInTerminal)(command, { timeoutMs: 60000 });
            const out = (res.output || '').slice(0, constants_1.TOOL_RESULT_MAX_CHARS);
            if (res.timedOut) {
...
const proposeChangesTool = {
    ...
    async execute(args, session) {
        ...
        if (!rawChanges.length)
            return { ok: false, output: 'changes 不能为空数组' };
        const changes = rawChanges
            .map(c => {
            const r = c;
            return {
                filePath: String(r.filePath ?? ''),
                type: (r.type === 'edit' || r.type === 'delete' ? r.type : 'write'),
        ...
        if (!changes.length)
            return { ok: false, output: '未解析到有效变更（每项需 filePath）' };
        // 校验全部变更
        const { validateChange } = require('../apply/applyManager');
        ...
        const proposal = { name, task: String(args.task ?? ''), createdAt: new Date().toISOString(), changes };
        const { saveProposal } = require('../apply/applyManager');
...
async function runAgentLoop(opts) {
    ...
    const maxIterations = opts.maxIterations
        ?? vscode.workspace.getConfiguration(constants_1.AGENT_LOOP_CONFIG).get(constants_1.AGENT_LOOP_CONFIG_KEYS.maxIterations, constants_1.AGENT_LOOP_DEFAULT_MAX_ITERATIONS);
    const timeoutMs = opts.timeoutMs
        ?? vscode.workspace.getConfiguration(constants_1.AGENT_LOOP_CONFIG).get(constants_1.AGENT_LOOP_CONFIG_KEYS.timeoutMs, constants_1.AGENT_LOOP_DEFAULT_TIMEOUT_MS);
    const allowCommands = vscode.workspace.getConfiguration(constants_1.AGENT_LOOP_CONFIG)
        .get(constants_1.AGENT_LOOP_CONFIG_KEYS.allowCommands, true);
    ...
    const tools = (opts.tools?.length
        ? exports.DEFAULT_TOOLS.filter(t => opts.tools.includes(t.name))
        : exports.DEFAULT_TOOLS)
    ...
    const routed = await (0, modelRouter_1.routeModel)({ preferred: opts.model, taskType: 'coding' });
    if (!routed) {
    ...
    const candidates = await (0, modelRouter_1.getModelCandidates)({ preferred: opts.model, taskType: 'coding' });
    ...
    let checkpointId;
    const checkpointEnabled = opts.checkpoint ?? vscode.workspace.getConfiguration(constants_1.AGENT_LOOP_CONFIG)
        .get(constants_1.AGENT_LOOP_CONFIG_KEYS.checkpoint, constants_1.AGENT_LOOP_DEFAULT_CHECKPOINT);
    if (checkpointEnabled) {
        try {
            ...
            logger_1.logger.warn('[AgentLoop] 创建检查点失败（继续运行）', err);
        }
    }
    const messages = [];
    if (opts.resumeFrom) {
    ...
    const modeNote = opts.planOnly
        ...
        : '';
    messages.push(vscode.LanguageModelChatMessage.User(`${exports.AGENT_LOOP_SYSTEM_PROMPT}\n\n${modeNote}【任务】${opts.task}`));
    ...
    outer: for (let i = 1; i <= maxIterations; i++) {
        ...
        for (let ai = 0; ai < candidates.length; ai++) {
            ...
            try {
                ...
                llmOk = true;
                (0, modelRouter_1.recordModelCall)(m.model.name, m.tier, true, Date.now() - start);
                break;
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                (0, modelRouter_1.recordModelCall)(m.model.name, m.tier, false, Date.now() - start, msg);
        ...
        for (const call of calls) {
            ...
            const resultText = (result.error ? `${result.output}\n[错误] ${result.error}` : result.output).slice(0, constants_1.TOOL_RESULT_MAX_CHARS);
            ...
            logger_1.logger.info(`[AgentLoop] iter ${i} ${call.name} ok=${result.ok}`);
        }
        if (i >= maxIterations) {
...
function buildHistoryContext(prev) {
    const lines = [
...
// ── 轨迹渲染与命令注册 ──────────────────────────────────────────
/** 渲染轨迹为 Markdown 文档 */
function renderTraceMarkdown(task, result) {
    const lines = [
        `# Agent 运行记录`,
        '',
        `- 任务：${task}`,
...
function persistRun(runDir, task, result, parentId, mode) {
...
function registerAgentLoop(context) {
    context.subscriptions.push(vscode.commands.registerCommand('kodrix.agent.plan', async () => {
        const task = await vscode.window.showInputBox({
            prompt: vscode_1.l10n.t('描述任务（计划模式：只读分析代码库，输出实施计划，不修改任何文件）'),
            placeHolder: vscode_1.l10n.t('例如：为 src/router 增加并发限流的实施方案'),
        });
        if (!task)
            return;
        ...
        const runDir = path.join(ws, constants_1.WORKSPACE_KODRIX_DIR, constants_1.AGENT_RUNS_DIR);
        fs.mkdirSync(runDir, { recursive: true });
        void vscode.window.showInformationMessage(vscode_1.l10n.t('Plan 模式分析中：{0}…', task.slice(0, 40)));
        ...
        const task = await vscode.window.showInputBox({
            prompt: vscode_1.l10n.t('描述 Agent 任务（Agent 将自主读/写/搜索/执行并迭代完成）'),
            placeHolder: vscode_1.l10n.t('例如：读取 src/utils.ts 中的 TODO，整理并写入 docs/todos.md'),
        });
        if (!task)
            return;
        const folder = vscode.workspace.workspaceFolders?.[0];
        ...
        const runDir = path.join(ws, constants_1.WORKSPACE_KODRIX_DIR, constants_1.AGENT_RUNS_DIR);
        fs.mkdirSync(runDir, { recursive: true });
        await vscode.window.showInformationMessage(vscode_1.l10n.t('Agent 任务已启动：{0}…（完成后自动打开记录）', task.slice(0, 40)));
        ...
        if (result.checkpointId) {
            await vscode.window.showInformationMessage(vscode_1.l10n.t('已创建检查点 {0}（可运行「Kodrix: 恢复检查点」回滚 Agent 改动）', result.checkpointId));
        }
        if (result.status === 'completed') {
            await vscode.window.showInformationMessage(vscode_1.l10n.t('Agent 任务完成（{0} 轮，{1}s）', result.iterations, (result.durationMs / 1000).toFixed(1)));
        }
        else {
            await vscode.window.showWarningMessage(vscode_1.l10n.t('Agent 任务未完成：{0}（{1}s）', result.status, (result.durationMs / 1000).toFixed(1)));
        }
    }), vscode.commands.registerCommand(constants_1.COMMANDS.agentResume, async () => {
        const folder = vscode.workspace.workspaceFolders?.[0];
        if (!folder)
            return;
        const runDir = path.join(folder.uri.fsPath, constants_1.WORKSPACE_KODRIX_DIR, constants_1.AGENT_RUNS_DIR);
        const runs = (0, threads_1.loadRuns)(runDir);
        if (!runs.length) {
            await vscode.window.showInformationMessage(vscode_1.l10n.t('暂无会话记录（先运行 Agent 任务）'));
            return;
        }
        const picked = await (0, threads_1.pickRun)(runs, vscode_1.l10n.t('选择要续聊的会话（同一节点多次续聊即分支）'));
        if (!picked)
            return;
        const task = await vscode.window.showInputBox({ prompt: vscode_1.l10n.t('续聊任务（将作为该会话的子分支继续）'), value: `继续：${picked.task}` });
        if (!task)
            return;
        await vscode.window.showInformationMessage(vscode_1.l10n.t('Agent 续聊已启动：{0}…', task.slice(0, 40)));
        const result = await runAgentLoop({ task, workspace: folder.uri.fsPath, resumeFrom: picked.result });
        ...
        if (result.status === 'completed') {
            await vscode.window.showInformationMessage(vscode_1.l10n.t('Agent 续聊完成（{0} 轮）', result.iterations));
        }
        else {
            await vscode.window.showWarningMessage(vscode_1.l10n.t('Agent 续聊未完成：{0}', result.status));
        }
        ...
        if (!fs.existsSync(runDir)) {
            await vscode.window.showInformationMessage(vscode_1.l10n.t('暂无 Agent 运行记录'));
            return;
        }
        ...
        const picked = await vscode.window.showQuickPick(files, { placeHolder: vscode_1.l10n.t('选择 Agent 运行记录') });
        if (!picked)
            return;
