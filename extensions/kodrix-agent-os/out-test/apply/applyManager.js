...
function validateChange(change, workspace) {
    ...
    if (!abs)
        return { ok: false, error: `路径越界：${change.filePath}` };
    if (!change.filePath.trim())
        return { ok: false, error: 'filePath 为空' };
    if (change.type === 'edit') {
        if (!change.oldContent)
            return { ok: false, error: `edit 变更缺少 oldContent：${change.filePath}` };
        if (!fs.existsSync(abs))
            return { ok: false, error: `文件不存在，无法 edit：${change.filePath}` };
        const cur = fs.readFileSync(abs, 'utf-8');
        const count = cur.split(change.oldContent).length - 1;
        if (count === 0)
            return { ok: false, error: `oldContent 未匹配：${change.filePath}` };
        if (count > 1)
            return { ok: false, error: `oldContent 匹配 ${count} 处（需唯一）：${change.filePath}` };
    }
...
function registerApplyManager(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(constants_1.COMMANDS.applyPreview, async () => {
        ...
        const ok = await vscode.window.showWarningMessage(`确认应用提案「${proposal.name}」（${proposal.changes.length} 个文件变更）？应用前将自动创建检查点。`, { modal: true }, '应用变更');
        if (ok !== '应用变更')
            return;
