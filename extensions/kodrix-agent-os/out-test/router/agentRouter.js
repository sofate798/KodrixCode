...
exports.routeAgentPrompt = routeAgentPrompt;
...
function getLastRoute() {
    return extensionContext?.workspaceState.get(LAST_ROUTE_KEY);
}
async function persistLastRoute(route, prompt) {
    if (!extensionContext) {
        return;
    }
    const record = {
...
async function routeAgentPrompt(prompt, options) {
    ...
    const autoExecute = vscode.workspace.getConfiguration('kodrix.router').get('autoExecute', true);
...
async function executeRoute(target, prompt) {
    try {
        switch (target) {
            case 'spec': {
                if ((0, specHelpers_1.listSpecSlugs)().length > 0) {
