/*---------------------------------------------------------------------------------------------
 *  Minicode — Cursor 对标能力：Tab 补全 / Agent / Composer / @Codebase / Cursor 3
 *--------------------------------------------------------------------------------------------*/
...
export function registerCursorFeatureCommands(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.buildCodebaseIndex', () => triggerCodebaseIndexBuild()),
		vscode.commands.registerCommand('minicode.attachCodebase', () => attachCodebaseToChat()),
		vscode.commands.registerCommand('minicode.openComposer', () => openComposerMode()),
		vscode.commands.registerCommand('minicode.continueInBackground', () => continueInBackgroundAgent()),
		vscode.commands.registerCommand('minicode.continueInCloud', () => continueInCloudAgent()),
		vscode.commands.registerCommand('minicode.openAgentsWindow', () => openAgentsWindow()),
		vscode.commands.registerCommand('minicode.openWorkspaceInAgentsWindow', () => openWorkspaceInAgentsWindow()),
		vscode.commands.registerCommand('minicode.applyCursorFeatures', async () => {
			await applyCursor3ExperienceDefaults(context, { force: true });
			vscode.window.showInformationMessage('Minicode：已应用 Cursor 3.0 Agent 中心体验默认配置');
		}),
		vscode.commands.registerCommand('minicode.applyCursor3Experience', async () => {
			await applyCursor3ExperienceDefaults(context, { force: true });
			await bootstrapCursor3WorkspaceLayout(context);
			vscode.window.showInformationMessage('Minicode：已应用 Cursor 3.0 完整体验并重载布局');
		}),
	...
	if (vscode.workspace.getConfiguration('minicode.features').get<boolean>('codebaseIndexAutoBuild', true)) {
