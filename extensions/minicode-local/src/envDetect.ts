...
async function detectTool(_name: string, cmd: string, timeoutMs = 4000): Promise<boolean> {
...
function detectCursorminiConfig(): boolean {
	...
	const minicode = path.join(os.homedir(), '.minicode');
	return fs.existsSync(miniHome) || fs.existsSync(minicode);
}
...
export async function detectEnvironment(): Promise<EnvInfo> {
	...
	const [gpu, ollama, python, hasGit, hasDocker] = await Promise.all([
		...
		detectOllama(),
		detectPython(),
		detectTool('git', 'git --version 2>&1'),
		detectTool('docker', 'docker --version 2>&1'),
	]);
