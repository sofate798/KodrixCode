	...
	switch (msg.command) {
		...
		case 'doImport': {
			try {
				...
				await context.globalState.update('minicode.cursorImported', true);
			} catch (err) {
		...
		case 'quickStart':
			switch (msg.action) {
				...
				case 'models':
					...
					break;
				case 'import':
					await vscode.commands.executeCommand('minicode.importCursor');
					break;
			}
			break;

		case 'finish':
			await context.globalState.update('minicode.welcomed', true);
			panel.dispose();
			break;
...
export async function openOnboardingWizard(context: vscode.ExtensionContext): Promise<void> {
	...
	context.subscriptions.push(
		...
		panel.onDidDispose(() => {
			...
			context.globalState.update('minicode.welcomed', true).then(
				undefined,
...
export function registerOnboarding(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.onboarding.open', () => openOnboardingWizard(context)),
	);
}
