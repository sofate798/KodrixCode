/*---------------------------------------------------------------------------------------------
 *  Minicode — 从 Cursor 导入 Rules / MCP / Skills / 设置
 *--------------------------------------------------------------------------------------------*/
...
const CURSOR_IMPORTED_KEY = 'minicode.cursorImported';
...
async function importCursorUserRulesFile(): Promise<string[]> {
	...
	const destRoot = path.join(os.homedir(), '.minicode', 'instructions');
	...
	if (count) {
		await mergeConfigLocations('instructionsFilesLocations', { '~/.minicode/instructions': true });
		applied.push(`用户 Rules（${count} 个）→ ~/.minicode/instructions`);
	}
	return applied;
}
...
async function importCursorUserRulesFromDatabase(): Promise<string[]> {
	...
	if (result.imported && result.destPath) {
		await mergeConfigLocations('instructionsFilesLocations', { '~/.minicode/instructions': true });
		applied.push(`SQLite User Rules（${result.source}）→ ~/.minicode/instructions/cursor-user-rules.instructions.md`);
	}
	return applied;
}
...
export async function importFromCursorOnFirstRun(context: vscode.ExtensionContext): Promise<void> {
	...
	const autoImport = vscode.workspace.getConfiguration('minicode').get<boolean>('importCursorOnFirstRun', true);
	if (!autoImport) {
		return;
	}
	const cursorHome = resolveCursorHomeDir();
