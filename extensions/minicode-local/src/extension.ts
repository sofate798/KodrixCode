/*---------------------------------------------------------------------------------------------
 *  Minicode Local — 本地/云端模型供应商与配置迁移
 *
 *  大厂工程化标准：
...
import { openProviderWorkbench } from './providerWorkbench';
import { registerMinicodeLanguageModels } from './languageModelProvider';
import { openOnboardingWizard, registerOnboarding } from './onboarding';

// ── 常量 ────────────────────────────────────────────────────────────

const MIGRATED_KEY = 'minicode.configMigrated';
const WELCOMED_KEY = 'minicode.welcomed';
/** 迁移重试计数键 */
const MIGRATE_ATTEMPTS_KEY = 'minicode.configMigrateAttempts';
/** 最大迁移重试次数，超过后不再尝试 */
...
async function showWelcome(context: vscode.ExtensionContext, migrated: boolean): Promise<void> {
	const choice = await vscode.window.showInformationMessage(
		migrated
			? '欢迎使用 Minicode — Cursor 3.0 以 Agent 为中心的全新编程体验。'
			: '欢迎使用 Minicode！并行 Agent、云端/本地大模型、Agents 窗口，精仿 Cursor 3.0。',
		{ modal: false },
		'配置 AI 模型',
		'Agents 窗口',
		'打开 Hub',
	...
	const commandMap: Record<string, string> = {
		'配置 AI 模型': 'minicode.openProviderWorkbench',
		'Agents 窗口': 'minicode.openAgentsWindow',
		'打开 Hub': 'minicode.hub.open',
		'Agent OS 概览': 'minicode.agentOs.welcome',
		'从 Cursor 导入': 'minicode.importCursor',
		'Skill 市场': 'minicode.skills.openMarketplace',
	};
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
	try {
		...
		logError('Minicode Local activation failed', err);
		vscode.window.showErrorMessage(`Minicode Local 激活失败: ${msg}`);
	}
}
...
async function activateInternal(context: vscode.ExtensionContext): Promise<void> {
	...
	registerMinicodeLanguageModels(context);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.migrateConfig', async () => {
		...
		vscode.commands.registerCommand('minicode.importCursor', async () => {
		...
		vscode.commands.registerCommand('minicode.openProviderPresets', async () => {
		...
		vscode.commands.registerCommand('minicode.openProviderWorkbench', () => {
			openProviderWorkbench(context);
		}),

		vscode.commands.registerCommand('minicode.applyPreset', async (presetId: string) => {
		...
		vscode.commands.registerCommand('minicode.welcome', async () => {
			await showWelcome(context, context.globalState.get<boolean>(MIGRATED_KEY, false));
		}),
	);

	// ── 首次迁移逻辑（带重试上限） ──
	const migrateOnFirstRun = vscode.workspace.getConfiguration('minicode').get<boolean>('migrateOnFirstRun', true);
	...
	if (migrateOnFirstRun && !alreadyMigrated) {
		...
		if (attempts < MAX_MIGRATE_ATTEMPTS) {
			try {
				...
				logError(`Minicode Local: migration attempt ${attempts + 1}/${MAX_MIGRATE_ATTEMPTS} failed`);
			}
		}
	}
