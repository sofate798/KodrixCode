/*---------------------------------------------------------------------------------------------
 *  Minicode — Cursor 3.0 / Cursor 风格默认体验配置
 *--------------------------------------------------------------------------------------------*/

import * as vscode from 'vscode';

const DEFAULTS_APPLIED_KEY = 'minicode.cursorDefaultsApplied';
...
const MINICODE_INSTRUCTION_LOCATIONS: Record<string, boolean> = {
	'~/.minicode/instructions': true,
	'.minicode/instructions': true,
	'.minicode/wiki': true,
	'.minicode/solo': true,
};
...
export async function applyCursorLikeDefaults(
	...
	for (const { section, key, value, force } of CURSOR_LIKE_DEFAULTS) {
		...
		if (force || current === undefined || (typeof current === 'boolean' && current === false && value === true)) {
			try {
				...
				console.warn(`[minicode-local] 跳过配置写入 ${section}.${key}:`, err instanceof Error ? err.message : err);
			}
		}
	}
	...
	await mergeLocationSetting('instructionsFilesLocations', MINICODE_INSTRUCTION_LOCATIONS, target);
...
export async function applyModelRoutes(): Promise<void> {
	const routes = vscode.workspace.getConfiguration('minicode').get<Record<string, { model?: string }>>('modelRoutes', {});
	...
	if (planRoute?.model) {
		await vscode.workspace.getConfiguration().update('chat.planAgent.defaultModel', planRoute.model, target);
	}
	if (agentRoute?.model) {
		await vscode.workspace.getConfiguration('github.copilot.chat').update('implementAgent.model', agentRoute.model, target);
	}
	if (codeRoute?.model) {
		await vscode.workspace.getConfiguration().update('chat.exploreAgent.defaultModel', codeRoute.model, target);
	}
	if (fastRoute?.model) {
		await vscode.workspace.getConfiguration().update('chat.utilitySmallModel', fastRoute.model, target);
	}
}
