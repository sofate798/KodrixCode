/*---------------------------------------------------------------------------------------------
 *  Minicode 语言模型 — 直接请求已配置的 OpenAI / Anthropic 兼容接口
 *
 *  不经过 GitHub Copilot 登录。Chat 模型选择器里厂商名为 Minicode。
 *--------------------------------------------------------------------------------------------*/
...
const VENDOR = 'minicode';

interface MinicodeModelInfo extends vscode.LanguageModelChatInformation {
	providerId: string;
	modelId: string;
	apiType: string;
	baseUrl: string;
...
export function notifyMinicodeModelsChanged(): void {
	onDidChangeEmitter.fire();
}

export function registerMinicodeLanguageModels(context: vscode.ExtensionContext): void {
	const provider: vscode.LanguageModelChatProvider<MinicodeModelInfo> = {
...
function toModelInfos(provider: StoredProvider): MinicodeModelInfo[] {
