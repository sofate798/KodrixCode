...
const _diag = vscode.window.createOutputChannel('Minicode Skills', { log: true });
...
export function resolveSkillsDir(): string {
	const configured = vscode.workspace.getConfiguration('minicode.skills').get<string>('installDir', '~/.agents/skills');
...
export async function installFromCatalogItem(
	...
	if (item.downloadUrl) {
		const tmpZip = path.join(os.tmpdir(), `minicode-skill-${Date.now()}.zip`);
		await downloadFile(item.downloadUrl, tmpZip);
		const extractDir = path.join(os.tmpdir(), `minicode-skill-extract-${Date.now()}`);
...
export async function installFromUrl(url: string): Promise<string> {
	...
	if (/github\.com/i.test(trimmed)) {
		...
		const tmpZip = path.join(os.tmpdir(), `minicode-gh-${Date.now()}.zip`);
		try {
		...
		const extractDir = path.join(os.tmpdir(), `minicode-gh-extract-${Date.now()}`);
		fs.mkdirSync(extractDir, { recursive: true });
		try {
			extractZip(tmpZip, extractDir);
		} finally {
...
function fetchText(url: string, _depth = 0): Promise<string> {
	...
	return new Promise((resolve, reject) => {
		...
		const req = lib.get(url, {
			headers: { 'User-Agent': 'Minicode-Skills/1.0' },
		}, res => {
