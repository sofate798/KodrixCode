/*---------------------------------------------------------------------------------------------
 *  统一 Agent OS 路由 — 自动选 SOLO / Spec / Plan / Agent / Ask（大厂智能分发）
 *--------------------------------------------------------------------------------------------*/

import * as vscode from 'vscode';
import { emitMinicodeEvent } from '../context/minicodeEventBus';
import { listSpecSlugs } from '../spec/specHelpers';
import { logger } from '../logger';

export type RouteTarget = 'solo' | 'spec' | 'plan' | 'agent' | 'ask';
...
const LAST_ROUTE_KEY = 'minicode.router.lastRoute';

const SOLO_PATTERNS: Array<{ pattern: RegExp; weight: number }> = [
	{ pattern: /@solo|solo\s*builder/i, weight: 3 },
	{ pattern: /从零|全新|搭建|创建.*(项目|应用|网站|系统)/, weight: 2 },
	{ pattern: /做一个.*(app|应用|网站|项目)/i, weight: 2 },
	{ pattern: /react.*vite|vue.*vite|fastapi|flask|静态.*页面/i, weight: 1 },
	{ pattern: /vibe\s*coding|快速原型|mvp/i, weight: 2 },
];
...
export function classifyIntent(prompt: string): RouteResult {
	...
	const scores: Array<{ target: RouteTarget; score: number; reason: string }> = [
		{ target: 'solo', score: scorePatterns(text, SOLO_PATTERNS), reason: '从零构建 / SOLO 场景' },
...
export async function routeAgentPrompt(prompt?: string, options?: { silent?: boolean }): Promise<void> {
	const enabled = vscode.workspace.getConfiguration('minicode.features').get<boolean>('agentRouter', true);
	if (!enabled) {
		vscode.window.showWarningMessage('智能路由已关闭。可在设置中启用 minicode.features.agentRouter');
		return;
	}
	...
	const autoExecute = vscode.workspace.getConfiguration('minicode.router').get<boolean>('autoExecute', true);
	...
	if (choice === '切换模式') {
		const picked = await vscode.window.showQuickPick(
			(['solo', 'spec', 'plan', 'agent', 'ask'] as RouteTarget[]).map(t => ({ label: t.toUpperCase(), target: t })),
			{ placeHolder: '手动选择模式' },
		);
		if (picked) {
...
export async function executeRoute(target: RouteTarget, prompt: string): Promise<void> {
	try {
		switch (target) {
			case 'solo':
				await vscode.commands.executeCommand('minicode.solo.start');
			...
			case 'spec': {
				if (listSpecSlugs().length > 0) {
					...
					await vscode.commands.executeCommand('minicode.spec.create');
				}
				break;
			}
			case 'plan':
 ...
 * `routeAgentPrompt`（受 `minicode.router.autoExecute` + confidence 控制）。
 */
...
export function registerRouter(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.router.route', () => routeAgentPrompt()),
		vscode.commands.registerCommand('minicode.router.execute', (target: RouteTarget, prompt: string) => executeRoute(target, prompt)),
		vscode.commands.registerCommand('minicode.router.repeatLast', async () => {
			const last = getLastRoute();
			if (!last) {
				vscode.window.showInformationMessage('尚无路由历史 — 使用 `Minicode: 智能路由` 开始');
				return;
			}
