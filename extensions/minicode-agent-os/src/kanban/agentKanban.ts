...
export class KanbanTreeItem extends vscode.TreeItem {
	constructor(
		...
		this.command = {
			command: 'minicode.kanban.openSession',
			title: '打开 Agent 会话',
			arguments: [this],
		};
	}
...
export function registerKanban(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		vscode.window.registerTreeDataProvider('minicode.agentKanban', provider),
		vscode.commands.registerCommand('minicode.kanban.addTask', async () => {
			await addKanbanTask();
			provider.refresh();
		}),
		vscode.commands.registerCommand('minicode.kanban.moveTask', async (item?: KanbanTreeItem) => {
			await moveKanbanTask(item);
			provider.refresh();
		}),
		vscode.commands.registerCommand('minicode.kanban.openSession', async (item?: KanbanTreeItem) => {
			await openKanbanSession(item);
			provider.refresh();
		}),
		vscode.commands.registerCommand('minicode.kanban.deleteTask', async (item?: KanbanTreeItem) => {
			await deleteKanbanTask(item);
			provider.refresh();
		}),
		vscode.commands.registerCommand('minicode.kanban.refresh', () => provider.refresh()),
		vscode.commands.registerCommand('minicode.kanban.focus', async () => {
			await vscode.commands.executeCommand('workbench.view.explorer');
			await vscode.commands.executeCommand('minicode.agentKanban.focus');
		}),
	);
}
