...
export async function registerInstructionFolders(): Promise<void> {
	const enabled = vscode.workspace.getConfiguration('minicode.features').get<boolean>('contextInjection', true);
	if (!enabled) {
		return;
	}
...
applyTo: '**'
description: Repo Wiki 架构摘要（Minicode Agent OS 自动生成）
---
...
> Qoder 风格 Repo Wiki · 由 Minicode Agent OS 维护
