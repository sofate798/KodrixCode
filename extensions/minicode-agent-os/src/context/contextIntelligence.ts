...
export function getContextSuggestions(): ContextSuggestion[] {
	...
	if (!wiki) {
		suggestions.push({
			...
			detail: '运行「Minicode: 生成 Repo Wiki」加速 Agent 理解项目',
...
export async function refreshAllContext(): Promise<void> {
	...
	await vscode.window.withProgress(
		{ location: vscode.ProgressLocation.Notification, title: 'Minicode: 刷新 Agent 上下文…' },
		async () => {
...
export async function showContextStatus(): Promise<void> {
	...
	const minicodeLocs = Object.entries(locations).filter(([k]) => k.includes('.minicode') || k.includes('minicode'));

	const lines = [
		'# Minicode 上下文状态（大厂级诊断）',
		'',
		'## 功能开关',
		'',
		`| 功能 | 状态 |`,
		...
		`| Session Learning | ${cfg.get('sessionLearning') ? '✓' : '✗'} |`,
		`| Semantic Memory | ${cfg.get('semanticMemory') !== false ? '✓' : '✗'} |`,
		`| Context 注入 | ${cfg.get('contextInjection') ? '✓' : '✗'} |`,
		'',
		'## 上下文数据',
		'',
		...
		'## Instructions 位置',
		'',
		...(minicodeLocs.length ? minicodeLocs.map(([k, v]) => `- \`${k}\` → ${v ? '✓ 启用' : '✗ 禁用'}`) : ['- （尚未注册 Minicode 路径）']),
		'',
		'## 智能建议',
		'',
		...(suggestions.length
...
export function getStatusBarText(): string {
	...
	return parts.length ? `Minicode: ${parts.join('·')}` : 'Minicode';
}
	...
	if (chatApi?.registerChatWorkspaceContextProvider) {
		const provider = {
			...
			provideWorkspaceChatContext: (_token: vscode.CancellationToken): WorkspaceChatContextItem[] => {
				...
				return [{
					label: 'Minicode 项目上下文（Wiki + Memory + Semantic）',
		...
		context.subscriptions.push(chatApi.registerChatWorkspaceContextProvider('minicode.agentOs', provider));
	}
}
