 ...
 *
 *  同时通过 MinicodeEventBus 向状态栏品牌指示器推送状态变化。
 *
 *  大厂工程化标准：
...
import { emitMinicodeEvent, onMinicodeEvent } from '../context/minicodeEventBus';
...
export function setAgentState(state: AgentState, sessionId?: string): void {
	...
	// and the Minicode Hub can react in real time.
	emitMinicodeEvent({
...
export function registerAgentStateListener(context: vscode.ExtensionContext): void {
	...
	subscribeToMinicodeEvents(context);
}

/**
 * Internal subscription to MinicodeEventBus for fine-grained state transitions.
 */
function subscribeToMinicodeEvents(context: vscode.ExtensionContext): void {
