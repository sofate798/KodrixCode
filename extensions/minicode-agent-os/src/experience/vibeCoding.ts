 ...
 * - 简单 / 明确的需求 → SOLO / Spec / Agent 快捷路径
 */
export async function vibeCode(prompt?: string): Promise<void> {
	const enabled = vscode.workspace.getConfiguration('minicode.features').get<boolean>('vibeCoding', true);
	if (!enabled) {
		vscode.window.showWarningMessage('Vibe Coding 已关闭。可在设置中启用 minicode.features.vibeCoding');
		return;
	}

	const input = prompt || await vscode.window.showInputBox({
		prompt: '💡 描述你想要的应⽤，AI 全自动构建 — 复杂想法走 Idea Flow，简单需求走快捷路径',
	...
	if (shouldUseIdeaFlow && vscode.workspace.getConfiguration('minicode.features').get<boolean>('ideaFlow', true)) {
		// Route to full Idea Flow pipeline — pass prompt directly to avoid double-input
		await vscode.commands.executeCommand('minicode.idea.start', input);
...
用户需求：${input}`;
	...
	if (route.target === 'solo') {
		await vscode.commands.executeCommand('minicode.solo.openWindow');
		await vscode.commands.executeCommand('workbench.action.chat.open', {
			query: `@solo ${vibeContext}`,
			isPartialQuery: false,
		});
		vscode.window.showInformationMessage('🎨 Vibe Coding → SOLO 四栏工作台已打开');
	} else if (route.target === 'spec') {
...
export async function quickVibe(): Promise<void> {
	const lastPrompt = await vscode.window.showQuickPick(
		[
			...
			{ label: '✏️ 自定义...', description: '输入你自己的描述', prompt: '' },
		],
...
export function registerVibeCoding(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.vibe.start', () => vibeCode()),
		vscode.commands.registerCommand('minicode.vibe.quick', () => quickVibe()),
	);
}
