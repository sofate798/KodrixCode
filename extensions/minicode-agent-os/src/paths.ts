...
export function getMinicodeDir(): string {
	return path.join(os.homedir(), '.minicode');
}

/**
 * 返回工作区的「主根」文件夹。所有工作区级持久化（.minicode/、memory hash）
...
export function getProjectHash(): string {
	...
	// 以主根路径派生 hash，与 getWorkspaceMinicodeDir 使用同一个 folder，
...
export function getWorkspaceMinicodeDir(): string | undefined {
	const folder = getPrimaryWorkspaceFolder();
	if (!folder) {
		return undefined;
	}
	return path.join(folder.uri.fsPath, '.minicode');
}
...
export function getMemoryDir(): string {
	return path.join(getMinicodeDir(), 'memory', getProjectHash());
}
...
export function getGlobalInstructionsDir(): string {
	return path.join(getMinicodeDir(), 'instructions');
}
...
export function getMemoryInstructionLocationKey(): string {
	return `~/.minicode/memory/${getProjectHash()}`;
}

export function getWikiInstructionLocationKey(): string {
	return '.minicode/wiki';
}

export function getSoloInstructionLocationKey(): string {
	return '.minicode/solo';
}
