...
const HOOK_INSTALLED_KEY = 'minicode.sessionLearningHookInstalled';
const HOOK_FILE_NAME = 'minicode-session-learning.json';
...
const DISTILL_SYSTEM = `你是 Minicode Learning Engine。从 Agent 编程会话 transcript 中提取 **0-3 条** 值得跨会话记住的项目知识。
...
function getSessionLearningConfig(): {
	...
	const features = vscode.workspace.getConfiguration('minicode.features');
	const cfg = vscode.workspace.getConfiguration('minicode.sessionLearning');
	return {
...
async function applyInsights(
	...
	if (mode === 'prompt') {
		...
		const choice = await vscode.window.showInformationMessage(
			`Minicode 从 Agent 会话提炼了 ${insights.length} 条项目知识，是否沉淀？\n\n${preview}`,
			'全部沉淀', '逐条选择', '忽略',
		);
		if (choice === '忽略' || !choice) {
			return 0;
...
export async function installSessionLearningHook(
	...
	const hookRel = '.minicode/hooks/scripts/session-learn.mjs';
	const hookConfig = {
		hooks: {
			Stop: [
				{
...
export async function showSessionLearningStatus(): Promise<void> {
	...
	const lines = [
		...
		'## 原理',
		'',
		'1. `.github/hooks/minicode-session-learning.json` 在 Agent **Stop** 时运行',
		'2. Hook 将会话 transcript 写入 `.minicode/sessions/pending/`',
...
export function registerSessionLearning(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.learn.installSessionHook', () => installSessionLearningHook(context)),
		vscode.commands.registerCommand('minicode.learn.sessionStatus', () => showSessionLearningStatus()),
		vscode.commands.registerCommand('minicode.learn.processPendingSessions', () => {
			scanPendingSessions();
