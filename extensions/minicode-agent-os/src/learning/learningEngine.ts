...
export type LearningSource = 'manual' | 'capture' | 'session' | 'wiki' | 'spec' | 'solo' | 'auto';
...
applyTo: '**'
description: Minicode 项目 Memory 与学习沉淀（自动同步，请勿手动改文件名）
---
...
export function recordLearning(
	...
	const enabled = vscode.workspace.getConfiguration('minicode.features').get<boolean>('learning', true);
	if (!enabled) {
		return {
			id: 'disabled',
	...
	emitMinicodeEvent({ type: 'learning.recorded', id: entry.id, category: entry.category });
	return entry;
}
...
export function syncProjectInstructionsFile(): void {
	...
	const learningSection = recent.length
		...
		: '- （尚无自动学习记录 — 使用 `Minicode: 沉淀项目知识` 或 `从对话捕获 Memory`）';

	const body = `${INSTRUCTIONS_FRONTMATTER}# Minicode 项目上下文（自动同步）

> 此文件由 Agent OS 自动生成并注入 Agent 指令。请编辑同目录下的 \`memory.md\` 修改长期 Memory。

## 项目 Memory

${memory.trim() || '（空 — 运行 `Minicode: 查看项目 Memory` 开始记录）'}
...
- 复杂任务前先查阅 Repo Wiki（\`.minicode/wiki/\`）
- SOLO 规划见 \`.minicode/solo/last-plan.md\`
...
export async function learnFromSelection(): Promise<void> {
	const enabled = vscode.workspace.getConfiguration('minicode.features').get<boolean>('learning', true);
	if (!enabled) {
		vscode.window.showWarningMessage('Learning Engine 已关闭。可在设置中启用 minicode.features.learning');
		return;
	}
...
export async function showLearningDashboard(context?: vscode.ExtensionContext): Promise<void> {
	...
	const htmlPath = path.join(context.extensionPath, 'resources', 'learning-dashboard.html');
	try {
		const html = fs.readFileSync(htmlPath, 'utf-8').replace(/\{\{cspSource\}\}/g, panel.webview.cspSource);
	...
	panel.webview.onDidReceiveMessage(msg => {
		...
		if (msg.command === 'capture') {
			void vscode.commands.executeCommand('minicode.learn.capture');
...
export function registerLearningEngine(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('minicode.learn.capture', () => learnFromSelection()),
		vscode.commands.registerCommand('minicode.learn.dashboard', () => showLearningDashboard(context)),
	);
