/*---------------------------------------------------------------------------------------------
 *  Kodrix — 已配置 AI 供应商持久化
 *--------------------------------------------------------------------------------------------*/

import * as vscode from 'vscode';
import { ProviderPreset, StoredProvider, ModelRoutes } from './types';

const PROVIDERS_KEY = 'kodrix.providers';
const ACTIVE_PROVIDER_KEY = 'kodrix.activeProviderId';
...
export function loadModelRoutes(): ModelRoutes {
	return vscode.workspace.getConfiguration('kodrix').get<ModelRoutes>('modelRoutes', {});
}

export async function saveModelRoutes(routes: ModelRoutes): Promise<void> {
	await vscode.workspace.getConfiguration('kodrix').update(
		'modelRoutes',
		routes,
