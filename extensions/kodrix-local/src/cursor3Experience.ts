/*---------------------------------------------------------------------------------------------
 *  Kodrix — Cursor 3.0 以 Agent 为中心的体验（Agents Window + 并行编排 + IDE 协同）
 *--------------------------------------------------------------------------------------------*/
...
export const CURSOR3_FEATURES_APPLIED_KEY = 'kodrix.cursorFeaturesVersion';
export const CURSOR3_WORKSPACE_BOOTSTRAP_KEY = 'kodrix.cursor3WorkspaceBootstrapped';
...
export async function applySettingEntries(
	...
	for (const { section, key, value } of entries) {
		...
		if (
			...
			await safeUpdateConfiguration(fullKey, value, target);
		}
	}
}
