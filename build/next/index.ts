...
/**
 * Write out/package.json to match the esbuild output module format.
 * Since the transpiler produces ESM output (format: 'esm'), the output
 * directory must declare "type": "module". This prevents stale build
 * mismatches where CommonJS output from an older compiler is loaded as
 * ESM due to the root package.json "type" field.
 */
async function writeOutPackageJson(outDir: string): Promise<void> {
	const outPkgPath = path.join(REPO_ROOT, outDir, 'package.json');
	await fs.promises.writeFile(outPkgPath, JSON.stringify({ type: 'module' }, null, '\t') + '\n', 'utf8');
	console.log(`[build] Wrote ${outDir}/package.json (type: module)`);
}
		...
		switch (command) {
			case 'transpile':
				if (options.watch) {
					...
					await writeOutPackageJson(outDir);
