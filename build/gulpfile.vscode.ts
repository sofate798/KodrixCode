...
function computeChecksum(filename: string): string {
	try {
		...
		const msg = err?.code === 'ENOENT'
			? `Checksum file not found: ${filename}. Ensure the build output is complete.`
			: `Failed to compute checksum for ${filename}: ${err?.message ?? err}`;
		throw new Error(msg);
	}
}
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			// Skip ELF/Mach-O .node leftovers (e.g. claude-agent-sdk audio-capture).
