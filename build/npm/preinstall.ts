...
/** VS year label → on-disk folder names (2026 installs under `18`, etc.) */
const VS_VERSION_FOLDERS: Record<string, string[]> = {
	'2026': ['2026', '18'],
	'2022': ['2022', '17'],
	'2019': ['2019', '16'],
};
...
function hasSupportedVisualStudioVersion() {
	...
	for (const version of supportedVersions) {
		...
		// Check default installation paths (including internal folder names like `18` for VS 2026)
		...
		const versionFolders = VS_VERSION_FOLDERS[version] ?? [version];
	...
	// Custom install location (e.g. D:\Program Files\...) — resolve via vswhere
	const vswherePath = findVisualStudioPathWithVswhere();
	return vswherePath ? 1 : 0;
}

function findVisualStudioPathWithVswhere(): string | undefined {
	const programFiles86Path = process.env['ProgramFiles(x86)'];
	if (!programFiles86Path) {
		return undefined;
	}
	...
	try {
		const installationPath = child_process.execFileSync(vswhere, [
			'-latest',
		...
		// No matching Visual Studio installation
	}
