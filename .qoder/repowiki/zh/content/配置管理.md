...
## 更新摘要
**变更内容**
- 新增 SecretStorage 安全存储机制，API 密钥不再以明文形式存储在 settings.json 中
- 实现 FIM API Key 和 Embedding API Key 的加密存储
- 添加设置页面支持，提供图形化界面管理敏感配置
- 更新配置迁移机制，支持从旧配置格式迁移到新的安全存储
- 增强配置验证和错误处理，确保密钥安全性
...
**重要更新**：Kodrix 现已集成 SecretStorage 安全存储机制，所有 API 密钥和敏感信息都通过加密存储，不再以明文形式保存在 settings.json 中，大幅提升配置安全性。

## 项目结构
Kodrix 的配置由"产品层 + 用户层 + 工作区级"构成，并通过构建脚本与扩展能力进行加载、校验与编辑支持。新增的 SecretStorage 机制为敏感配置提供了安全的存储方案。
...
## 架构总览
下图展示产品配置如何贯穿构建期与运行期，并影响扩展生态、默认代理与安全策略。新增的 SecretStorage 机制为敏感配置提供了独立的安全存储层。
...
- [SECURITY.md:46-63](file://SECURITY.md#L46-L63)
- [package.json:864-882](file://extensions/kodrix-agent-os/package.json#L864-L882)
...
## 结论
Kodrix 的配置体系以 product.json 为核心，结合用户与工作区设置形成清晰的优先级与覆盖机制。**重大更新**：新增的 SecretStorage 安全存储机制为敏感配置提供了企业级的安全保障，彻底解决了 API 密钥明文存储的安全隐患。通过构建期注入、扩展生态配置与安全策略，实现品牌、功能与安全的统一管理。遵循最佳实践与故障排查流程，可显著提升团队的配置一致性与可维护性。
