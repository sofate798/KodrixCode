# 持续集成

<cite>
**本文引用的文件**   
- [build.yml](file://.github/workflows/build.yml)
- [pr-check.yml](file://.github/workflows/pr-check.yml)
- [nightly.yml](file://.github/workflows/nightly.yml)
- [lint-zizmor.yml](file://.github/workflows/lint-zizmor.yml)
- [dependabot.yml](file://.github/dependabot.yml)
- [copilot-instructions.md](file://.github/copilot-instructions.md)
- [package.json](file://package.json)
</cite>

## 更新摘要
**所做更改**   
- 更新了工作流架构，从12个独立文件简化为统一的构建流程
- 将分支策略从main更新为master
- 简化了夜间构建，专注于编译检查和依赖审计而非完整测试套件
- 集成了Gitleaks安全扫描与npm audit（仅critical级别）
- 更新了PR检查流程和安全扫描配置

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能与可维护性](#性能与可维护性)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介
本文件系统性梳理 Kodrix 的持续集成与持续交付（CI/CD）体系，覆盖 GitHub Actions 工作流、Pull Request 检查、自动化测试、代码质量与安全扫描、依赖更新管理、发布流水线以及常见问题的定位与修复建议。目标是帮助团队快速理解并高效使用现有 CI/CD 能力，同时为后续扩展提供清晰参考。

## 项目结构
Kodrix 的 CI/CD 配置集中在 .github 目录下，经过重大重构后采用统一的工作流架构：
- 统一构建流程：build.yml 处理三平台构建
- PR 检查与基础质量门禁：PR Check
- 夜间质量保障：Nightly Quality
- 安全扫描：Lint Zizmor
- 依赖安全与更新治理（Dependabot）
- AI 辅助开发规范（Copilot 指令）

```mermaid
graph TB
subgraph "GitHub Actions"
BUILD["统一构建<br/>三平台构建/打包/产物上传"]
PR["PR Check<br/>静态检查/类型检查/依赖审计/扩展测试"]
NIGHTLY["Nightly Quality<br/>编译检查/依赖审计"]
SECURITY["Security Scan<br/>Zizmor/Gitleaks/npm audit"]
end
subgraph "外部系统"
GH["GitHub Releases/Artifacts"]
AUR["AUR 包仓库"]
SNAP["Snap Store"]
GITLAB["GitLab Pages 触发器"]
end
BUILD --> GH
PR --> GH
NIGHTLY --> GH
SECURITY --> GH
```

**图示来源**
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)

**章节来源**
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)

## 核心组件
- **统一构建流程**：支持 Windows x64、Linux x64、macOS arm64 三平台构建
- **Pull Request 检查**：ESLint + TypeScript 编译检查、扩展单元测试、安全扫描
- **夜间质量保障**：全量编译检查、依赖审计、过时依赖报告
- **安全扫描**：Zizmor 工作流安全检查、Gitleaks 密钥扫描、npm audit critical 级别
- **依赖更新管理**：Dependabot 每周自动更新 GitHub Actions 依赖
- **AI 辅助开发规范**：Copilot 指令约束编码标准、安全约定和测试要求

**章节来源**
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)
- [dependabot.yml:1-10](file://.github/dependabot.yml#L1-L10)
- [copilot-instructions.md:1-88](file://.github/copilot-instructions.md#L1-L88)

## 架构总览
下图展示从代码提交到发布的端到端流程，包括 PR 门禁、统一构建、制品归档、安全扫描与依赖治理。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant GH as "GitHub"
participant PRW as "PR Check"
participant BL as "统一构建"
participant NQ as "夜间质量"
participant SEC as "安全扫描"
Dev->>GH : 推送/创建 PR
GH->>PRW : 触发 PR Check (master分支)
PRW-->>Dev : 静态检查/类型检查/审计/测试报告
GH->>BL : 触发构建 (v*标签)
BL-->>GH : 上传三平台构建产物
GH->>NQ : 定时触发夜间质量检查
NQ-->>GH : 编译检查/依赖审计报告
GH->>SEC : 工作流安全检查
SEC-->>GH : 安全扫描结果
```

**图示来源**
- [pr-check.yml:3-11](file://.github/workflows/pr-check.yml#L3-L11)
- [build.yml:13-17](file://.github/workflows/build.yml#L13-L17)
- [nightly.yml:3-6](file://.github/workflows/nightly.yml#L3-L6)
- [lint-zizmor.yml:3-14](file://.github/workflows/lint-zizmor.yml#L3-L14)

## 详细组件分析

### Pull Request 检查流程
**更新** 分支策略已从 main 更新为 master，安全检查已简化为仅 critical 级别的 npm audit

- 触发条件：对 master 分支的 PR 与 push
- 并发控制：同 PR 取消进行中任务，避免重复执行
- Lint 与类型检查：ESLint + TypeScript 编译检查
- 依赖审计：npm audit（仅 critical 严重级别告警）
- 扩展单元测试：针对 kodrix-agent-os、kodrix-local、kodrix-skills 并行执行
- 安全扫描：npm audit critical + gitleaks 密钥扫描

```mermaid
flowchart TD
Start(["PR/Push 触发"]) --> Checkout["检出代码"]
Checkout --> SetupNode["安装 Node.js 与缓存"]
SetupNode --> InstallDeps["npm ci"]
InstallDeps --> ESLint["运行 ESLint"]
ESLint --> TSCheck["TypeScript 编译检查"]
TSCheck --> AuditCritical["npm audit --audit-level=critical"]
AuditCritical --> TestExt["并行运行扩展单元测试"]
TestExt --> SecretScan["gitleaks 密钥扫描"]
SecretScan --> End(["产出报告/标记状态"])
```

**图示来源**
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)

**章节来源**
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)

### 统一构建流程（三平台）
**更新** 替代了原有的多个独立构建文件，现在通过单一 build.yml 文件处理所有平台的构建

- **Windows x64**：windows-2022 环境，构建 vscode-win32-x64-min
- **Linux x64**：ubuntu-22.04 环境，构建 vscode-linux-x64-min  
- **macOS arm64**：macos-14 环境，构建 vscode-darwin-arm64-min
- 产物格式：tar.gz 压缩包，包含完整可执行程序
- 构建优化：跳过 Node 版本检查、Playwright 浏览器下载、代码签名等耗时步骤

```mermaid
graph LR
Matrix["矩阵配置<br/>Windows/Linux/macOS"] --> Build["平台特定构建"]
Build --> Pack["产物打包"]
Pack --> Upload["上传制品"]
Upload --> Artifacts["GitHub Artifacts"]
```

**图示来源**
- [build.yml:31-52](file://.github/workflows/build.yml#L31-L52)
- [build.yml:98-124](file://.github/workflows/build.yml#L98-L124)

**章节来源**
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)

### 夜间质量保障
**更新** 简化了夜间任务，专注于编译检查和依赖审计，移除了完整的测试套件执行

- **全量编译检查**：执行 gulp compile-build-with-mangling 确保完整构建
- **依赖审计**：npm audit critical 级别安全扫描
- **过时依赖报告**：npm-check-updates 生成依赖更新报告（不阻断构建）
- 调度频率：每天 UTC 04:00（北京时间 12:00）

```mermaid
flowchart TD
Schedule["定时触发<br/>每天 UTC 04:00"] --> Compile["全量编译检查"]
Schedule --> DepCheck["依赖审计"]
Compile --> Report["构建报告"]
DepCheck --> Audit["npm audit critical"]
DepCheck --> Outdated["过时依赖报告"]
```

**图示来源**
- [nightly.yml:3-44](file://.github/workflows/nightly.yml#L3-L44)

**章节来源**
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)

### 安全扫描与合规检查
**更新** 集成了多种安全扫描工具，提供更全面的安全保障

- **Zizmor 工作流安全检查**：检测 GitHub Actions 工作流中的安全问题
- **Gitleaks 密钥扫描**：检测代码中的敏感信息和硬编码密钥
- **npm audit critical**：仅报告 critical 级别的依赖漏洞
- 权限最小化：zizmor 工作流使用空权限集合

```mermaid
flowchart TD
Code["代码提交"] --> Zizmor["Zizmor 工作流安全检查"]
Code --> Gitleaks["Gitleaks 密钥扫描"]
Code --> Audit["npm audit critical"]
Zizmor --> SecurityReport["安全报告"]
Gitleaks --> SecurityReport
Audit --> SecurityReport
```

**图示来源**
- [lint-zizmor.yml:18-31](file://.github/workflows/lint-zizmor.yml#L18-L31)
- [pr-check.yml:59-80](file://.github/workflows/pr-check.yml#L59-L80)

**章节来源**
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)
- [pr-check.yml:59-80](file://.github/workflows/pr-check.yml#L59-L80)

### 依赖更新管理（Dependabot）
- 监控范围：GitHub Actions 依赖
- 目标分支：insider
- 调度频率：每周一次
- 冷却期：默认 7 天，降低频繁 PR 噪音

```mermaid
flowchart TD
Cron["每周计划任务"] --> Scan["Dependabot 扫描 GitHub Actions 依赖"]
Scan --> PR["生成更新 PR(目标分支: insider)"]
PR --> Review["团队审查合并"]
```

**图示来源**
- [dependabot.yml:1-10](file://.github/dependabot.yml#L1-L10)

**章节来源**
- [dependabot.yml:1-10](file://.github/dependabot.yml#L1-L10)

### 代码审查辅助（Copilot 指令与规则）
- 编码规范：TypeScript strict、禁止 any、import type、禁用 console.log/warn、异步 I/O、资源释放
- 安全约定：路径遍历防护、命令注入防护、网络请求超时与大小限制、禁止硬编码密钥
- 测试要求：新增模块需单元测试；扩展集成测试位于指定目录；提交前运行扩展测试
- 验证清单：ESLint 零警告、编译通过、disposables 注册、无同步 I/O、计时器清理、命令文档化、配置变更反映在 package.json

```mermaid
flowchart TD
Code["新代码"] --> Rules["Copilot 指令约束"]
Rules --> Lint["ESLint 检查"]
Rules --> Type["TypeScript 严格模式"]
Rules --> Test["单元测试/集成测试"]
Rules --> Security["安全检查清单"]
Lint --> PR["PR 检查"]
Type --> PR
Test --> PR
Security --> PR
```

**图示来源**
- [copilot-instructions.md:31-88](file://.github/copilot-instructions.md#L31-L88)
- [pr-check.yml:18-33](file://.github/workflows/pr-check.yml#L18-L33)

**章节来源**
- [copilot-instructions.md:1-88](file://.github/copilot-instructions.md#L1-L88)
- [pr-check.yml:18-33](file://.github/workflows/pr-check.yml#L18-L33)

## 依赖分析
- 工作流耦合关系
  - PR Check 与 Nightly 均依赖 npm 脚本与 Node 环境
  - 统一构建工作流依赖工具链（GCC/Node/Python）与中间产物
  - 安全扫描工作流独立运行，提供额外的安全保障
- 外部依赖
  - GitHub Releases/Artifacts
  - Dependabot 依赖更新服务
  - Gitleaks 安全扫描服务
  - Zizmor 工作流安全检查服务

```mermaid
graph TB
PRW["PR Check"] --> NPM["npm 脚本/Node 环境"]
NIGHTLY["Nightly"] --> NPM
BUILD["统一构建"] --> TOOLCHAIN["GCC/Node/Python"]
BUILD --> ARTIFACTS["构建产物"]
SECURITY["安全扫描"] --> TOOLS["Gitleaks/Zizmor"]
DEPENDABOT["Dependabot"] --> ACTIONS["GitHub Actions 依赖"]
```

**图示来源**
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)
- [dependabot.yml:1-10](file://.github/dependabot.yml#L1-L10)

**章节来源**
- [pr-check.yml:1-80](file://.github/workflows/pr-check.yml#L1-L80)
- [nightly.yml:1-44](file://.github/workflows/nightly.yml#L1-L44)
- [build.yml:1-125](file://.github/workflows/build.yml#L1-L125)
- [lint-zizmor.yml:1-31](file://.github/workflows/lint-zizmor.yml#L1-L31)
- [dependabot.yml:1-10](file://.github/dependabot.yml#L1-L10)

## 性能与可维护性
- **并发与取消**：PR Check 启用 concurrency 取消进行中任务，减少资源浪费
- **矩阵构建**：三平台矩阵构建提升覆盖率，但会增加构建时长
- **缓存策略**：Node 依赖缓存加速安装
- **失败容忍**：部分审计步骤设置 continue-on-error，避免阻塞关键流程
- **环境变量控制**：通过环境变量控制构建优化选项（如跳过代码签名、浏览器下载等）

**优化建议**
- 利用统一构建流程的优势，进一步优化各平台构建时间
- 引入增量构建与缓存分层（如 Rust/Python 依赖缓存）
- 对长尾任务增加重试与超时保护
- 统一日志格式与结构化输出，便于检索与分析

## 故障排除指南
- **PR Check 失败**
  - ESLint 报错：根据错误提示修复代码风格问题，确保零警告
  - TypeScript 编译失败：检查类型定义与 import type 的使用
  - npm audit critical：评估风险或升级依赖
  - 扩展单元测试失败：查看对应扩展的测试结果工件
  - gitleaks 发现密钥：移除硬编码密钥，改用 secrets 管理
- **构建失败**
  - 工具链缺失：确认 GCC/Node/Python 版本与环境
  - 依赖安装失败：检查网络代理与镜像源
  - 多架构构建失败：确认容器镜像与架构匹配
  - 产物上传失败：检查权限与存储空间
- **夜间任务失败**
  - 编译检查失败：检查 gulp 任务配置与依赖
  - 依赖审计失败：评估 critical 级别漏洞风险
  - 过时依赖报告：根据报告更新依赖版本

**排查要点**
- 查看工作流日志中的 step 输出与错误堆栈
- 检查环境变量与 secrets 是否正确注入
- 使用本地复现脚本（scripts/test-*.sh）缩小问题范围
- 关注环境变量开关对构建的影响

**章节来源**
- [pr-check.yml:18-80](file://.github/workflows/pr-check.yml#L18-L80)
- [build.yml:69-124](file://.github/workflows/build.yml#L69-L124)
- [nightly.yml:13-44](file://.github/workflows/nightly.yml#L13-L44)

## 结论
Kodrix 的 CI/CD 体系经过重大重构后，采用了更加简洁高效的统一架构。通过单一构建文件处理三平台构建，简化了维护复杂度；PR 检查流程专注于快速反馈；夜间任务聚焦于编译检查和依赖审计；安全扫描提供了多层次的保护。这种设计在保证代码质量的同时，显著提升了构建效率和可维护性。建议在保持现有稳定性的基础上，继续优化构建缓存策略，增强日志可观测性，进一步提升流水线效率。