...
- [extensions/kodrix-agent-os/src/test/runTests.ts](file://extensions/kodrix-agent-os/src/test/runTests.ts)
- [extensions/kodrix-agent-os/out-test/test/runTests.js](file://extensions/kodrix-agent-os/out-test/test/runTests.js)
...
## 更新摘要
**所做更改**
- 新增每个扩展独立Mocha测试运行器的详细说明
- 更新测试架构以反映扩展级别的测试基础设施增强
- 添加Kodrix扩展的独立测试运行器实现细节
- 完善测试执行流程，包含扩展级测试运行器的集成方式
...
**更新** 本次更新重点反映了测试框架的重大增强：每个扩展现在都拥有独立的Mocha测试运行器，包含完整的测试基础设施和具体的测试实现。
...
- [extensions/kodrix-agent-os/src/test/runTests.ts:1-60](file://extensions/kodrix-agent-os/src/test/runTests.ts#L1-L60)
...
participant ExtTest as "扩展测试(独立Mocha)"
...
alt 扩展测试
Runner->>ExtTest : 运行独立Mocha测试
ExtTest-->>Artifacts : 测试结果/报告
end
alt 冒烟测试
...
- [extensions/kodrix-agent-os/src/test/runTests.ts:24-51](file://extensions/kodrix-agent-os/src/test/runTests.ts#L24-L51)
...
### 扩展级测试（Extension Tests）
- 目标与范围
  - 为每个扩展提供独立的测试运行器，确保扩展功能的完整测试覆盖
- 运行架构
  - 独立的Mocha测试运行器，不依赖 @vscode/test-electron
  - 自动发现并执行所有 .test.js 文件
  - 内置 vscode mock 支持，无需完整 VS Code 环境
- 实现特点
  - TDD 风格的测试接口
  - 10秒默认超时配置
  - 递归扫描测试文件
  - 统一的错误处理和退出码管理
- 支持的扩展
  - kodrix-agent-os：智能代理操作系统扩展
  - kodrix-local：本地模型支持扩展  
  - kodrix-skills：技能市场扩展

**新增** 每个扩展现在都有独立的Mocha测试运行器，提供完整的测试基础设施。

```mermaid
flowchart TD
A["扩展测试入口"] --> B["初始化Mocha运行器"]
B --> C["注册vscode mock"]
C --> D["递归查找.test.js文件"]
D --> E["添加到Mocha测试套件"]
E --> F["执行测试用例"]
...
图示来源
- [extensions/kodrix-agent-os/src/test/runTests.ts:24-51](file://extensions/kodrix-agent-os/src/test/runTests.ts#L24-L51)
- [extensions/kodrix-agent-os/out-test/test/runTests.js:63-89](file://extensions/kodrix-agent-os/out-test/test/runTests.js#L63-L89)

章节来源
- [extensions/kodrix-agent-os/src/test/runTests.ts:1-60](file://extensions/kodrix-agent-os/src/test/runTests.ts#L1-L60)
- [extensions/kodrix-agent-os/out-test/test/runTests.js:1-94](file://extensions/kodrix-agent-os/out-test/test/runTests.js#L1-L94)
- [extensions/kodrix-local/package.json:275-280](file://extensions/kodrix-local/package.json#L275-L280)
- [extensions/kodrix-skills/package.json:124-129](file://extensions/kodrix-skills/package.json#L124-L129)
...
## 依赖分析
- 测试框架耦合关系
  - Mocha：贯穿 unit、integration 与扩展测试
...
**更新** 扩展级测试现在使用独立的Mocha运行器，减少了对 @vscode/test-electron 的依赖。
...
- **扩展测试调试**
  - 检查 vs code-mock 是否正确注册
  - 确认测试文件命名符合 .test.js 规范
  - 验证递归文件扫描是否正常工作
...
## 结论
Kodrix 的测试体系以分层清晰、工具链完备为核心特征：Mocha 保障单元、集成与扩展测试的可控性，Playwright 支撑冒烟与发布健康检查的可靠性，VS Code Test API 打通扩展与集成测试的调试体验。**重大改进**是每个扩展现在都拥有独立的Mocha测试运行器，提供了更灵活、更快速的测试执行环境。通过合理的用例组织、严格的等待与隔离策略、以及 CI/CD 中的矩阵化执行，能够持续保障产品质量与交付效率。建议在此基础上进一步完善性能基线、数据工厂与环境标准化，形成更稳健的质量闭环。
