...
## 更新摘要
**变更内容**   
- 新增 BM25 全文检索实现，提供零依赖的文本相似度计算
- 实现可插拔 Embedding Provider 架构，支持外部向量模型集成
- 引入块级语义搜索功能，对标 Cursor 的语法块 embedding 能力
- 增强多语言支持，扩展 AST 解析器覆盖范围
- 优化缓存机制和性能策略，提升大规模项目处理效率
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [BM25 全文检索实现](#bm25-全文检索实现)
7. [嵌入提供者架构](#嵌入提供者架构)
8. [块级语义搜索](#块级语义搜索)
9. [依赖关系分析](#依赖关系分析)
10. [性能与优化](#性能与优化)
11. [多语言支持](#多语言支持)
12. [增量更新与变更检测](#增量更新与变更检测)
13. [故障排除指南](#故障排除指南)
14. [结论](#结论)
...
## 块级语义搜索
块级语义搜索是对标 Cursor syntax-block embedding 的核心功能：

- **代码块提取**：buildBlockDoc 函数根据符号位置信息提取完整代码块
- **精确位置**：利用 TS AST 提供的 endLine 信息进行精确截取
- **启发式扫描**：无精确位置时使用缩进、空白行、最大行数限制进行启发式扫描
- **质量优化**：截断至 200 行，避免过大块影响 embedding 质量
- **双重检索**：同时支持 Embedding 余弦相似度和 BM25 文本匹配
- **结果丰富**：返回代码块文本、符号信息、行号范围等完整上下文

**章节来源**
- [semanticIndex.ts:289-329](file://extensions/kodrix-agent-os/src/codebase/semanticIndex.ts#L289-L329)
...
## 结论
该语义索引子系统以 ProjectIndex 为核心，结合 BM25 全文检索、可插拔 Embedding 架构和块级语义搜索，提供了高性能、可扩展、多语言支持的代码理解能力。通过类型化设计、智能缓存与增量更新机制，系统在大规模仓库中仍能保持毫秒级响应与稳定可用性。
