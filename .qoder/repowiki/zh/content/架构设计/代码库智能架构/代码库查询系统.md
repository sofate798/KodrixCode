...
- 主动上下文感知（打开文件时提示相关记忆）：`extensions/kodrix-agent-os/src/context/proactiveContext.ts`
- 多层上下文组装与状态展示：`extensions/kodrix-agent-os/src/context/contextIntelligence.ts`
- 多 Agent 协作编排（并行执行引擎）：`extensions/kodrix-agent-os/src/crew/agentCrew.ts`
...
- 自然语言查询入口
  ...
  - 集成入口：注册 VS Code Chat Participant 与命令，提供对话式与自然语言问答体验。

- 语义记忆与向量检索
  - 本地 TF-IDF 词向量编码 + 余弦相似度，固定维度（128），零外部依赖。
  - 持久化向量索引文件，按学习日志条目维护，自动清理无效向量。
  - 内存缓存 entries，避免重复读盘；写入后失效缓存。
...
- 多 Agent 协作编排
  ...
  - 跨 Agent 上下文传递：上游任务输出注入下游 prompt，共享上下文文件。
  - 执行报告：Markdown 汇总任务明细、错误、耗时、待办提示。
...
Q->>S : "searchSymbolsAsync / searchFilesAsync"
S-->>Q : "语义命中含 score"
Q->>I : "符号名精确/模糊匹配"
I-->>Q : "符号结果"
end
...
- 查询重写
  ...
  - 通用语义检索包括：符号级语义检索、文件级语义检索、符号名精确/模糊匹配，并进行去重与截断。
...
### 向量检索实现原理
- 嵌入生成
  - 使用本地 TF-IDF 词向量编码（encodeText），固定维度 128，无需外部依赖。
  - 向量索引持久化为 JSON 文件，按学习日志条目维护，自动清理无效向量。
- 相似度计算
...
```mermaid
flowchart TD
VStart["输入查询"] --> Encode["TF-IDF 编码 encodeVector"]
Encode --> LoadIdx["加载向量索引 loadIndex"]
