...
## 更新摘要
**变更内容**
- 完成全面的异步I/O迁移，所有文件系统操作现在使用 `fs.promises` API
- 显著提升了性能和响应性，完全避免了UI冻结问题
- 更新了 checkpointManager.ts、checkpointDiffGallery.ts 和 checkpointTimelineView.ts 的异步实现细节
- 改进了函数签名、Promise处理机制和错误处理机制
- 优化了大型项目的处理能力和用户体验
...
## 简介
本系统为 Kodrix Agent OS 的"检查点可视化"能力，提供代码变更的可回滚、时间线查看与差异对比。它通过自动捕获与手动创建检查点，持久化工作区文件快照，并在侧边栏以树形视图展示；同时提供 Webview 画廊进行多检查点对比、逐文件接受/拒绝与批量回滚。**重大更新**：系统已全面升级为异步I/O架构，所有文件系统操作均使用 `fs.promises` API，显著提升了性能和响应性，完全避免了UI冻结问题。该能力与扩展主入口集成，在激活时注册命令、视图与事件监听。
