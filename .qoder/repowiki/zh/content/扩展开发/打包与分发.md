...
- [build/npm/postinstall.ts](file://build/npm/postinstall.ts)
</cite>

## 更新摘要
**变更内容**
- 更新了构建优化部分，反映移除@parcel/watcher依赖后的改进
- 新增了原生模块编译优化的详细说明
- 更新了依赖管理策略，消除了不必要的原生模块编译需求
- 增强了构建速度和包体积优化的指导
...
**最新更新**：本项目已优化构建流程，通过移除@parcel/watcher依赖消除了原生模块编译需求，显著提升了构建速度并减少了包体积。
...
- build/npm 包含自定义的安装前和安装后处理脚本，用于优化依赖安装过程。
...
A --> F["build/npm/*.ts<br/>构建优化脚本"]
B --> G["kodrix-agent-os<br/>命令/视图/设置等贡献点"]
B --> H["kodrix-local<br/>供应商设置/快捷键"]
B --> I["kodrix-skills<br/>技能市场视图与命令"]
F --> J["移除@parcel/watcher预编译模块<br/>优化构建性能"]
```
...
- [build/npm/postinstall.ts:143-157](file://build/npm/postinstall.ts#L143-L157)
...
- 构建优化：自定义的 preinstall 和 postinstall 脚本提供智能依赖管理和原生模块优化。

**更新**：新增构建优化组件，通过智能依赖管理消除不必要的原生模块编译。
...
- [package.json:12-100](file://package.json#L12-L100)
- [product.json:54-63](file://product.json#L54-L63)
- [build/npm/preinstall.ts:143-164](file://build/npm/preinstall.ts#L143-L164)
- [build/npm/postinstall.ts:240-314](file://build/npm/postinstall.ts#L240-L314)
...
**更新**：构建流程现已集成智能依赖管理，自动移除不必要的原生模块预编译。
...
### 构建优化与依赖管理
**新增**：项目实现了智能的依赖管理机制，通过自定义的 preinstall 和 postinstall 脚本优化构建过程。

#### 预安装阶段优化
- Node.js 版本检查和兼容性验证
- Visual Studio 工具链检测（Windows 平台）
- 智能缓存机制，避免重复安装
- 原生头文件自动下载和配置

#### 安装后优化
- 并行依赖安装，提升安装速度
- 智能移除 @parcel/watcher 预编译模块
- 容器化环境支持
- 环境变量隔离和配置管理

```mermaid
flowchart TD
PreInstall["preinstall.ts"] --> VersionCheck["Node.js 版本检查"]
VersionCheck --> VSCheck["Visual Studio 检测"]
VSCheck --> CacheCheck["依赖缓存检查"]
CacheCheck --> HeaderInstall["原生头文件安装"]
PostInstall["postinstall.ts"] --> ParallelInstall["并行依赖安装"]
ParallelInstall --> RemoveWatcher["移除@parcel/watcher预编译"]
RemoveWatcher --> ContainerSupport["容器环境支持"]
ContainerSupport --> ConfigCleanup["配置清理"]
```

**图表来源**
- [build/npm/preinstall.ts:11-164](file://build/npm/preinstall.ts#L11-L164)
- [build/npm/postinstall.ts:240-314](file://build/npm/postinstall.ts#L240-L314)
- [build/npm/postinstall.ts:143-157](file://build/npm/postinstall.ts#L143-L157)
...
**更新**：本地开发体验得到显著提升，构建优化脚本确保依赖安装更加快速和可靠。
...
- **新增**：智能依赖管理系统自动处理原生模块优化，减少不必要的编译开销。
...
Root --> BuildOpt["build/npm/*.ts<br/>构建优化脚本"]
BuildOpt --> WatcherOpt["@parcel/watcher 优化"]
```
...
- **新增**：智能依赖管理消除原生模块编译需求，显著提升构建速度和减少包体积。
- **新增**：并行依赖安装和智能缓存机制，大幅缩短安装时间。

**更新**：通过移除@parcel/watcher依赖和优化构建流程，构建性能得到显著改善。
...
- **新增**：依赖安装问题：检查构建优化脚本是否正常执行，确认@parcel/watcher预编译模块是否被正确移除。
- **新增**：原生模块编译错误：确认 Visual Studio 工具链配置正确，检查 node-gyp 环境设置。
...
- [.github/workflows/publish-stable-spearhead.yml:88-95](file:.github/workflows/publish-stable-spearhead.yml#L88-L95)
- [.github/workflows/publish-insider-spearhead.yml:88-95](file:.github/workflows/publish-insider-spearhead.yml#L88-L95)
- [build/npm/preinstall.ts:150-164](file://build/npm/preinstall.ts#L150-L164)
- [build/npm/postinstall.ts:143-157](file://build/npm/postinstall.ts#L143-L157)

## 结论
本仓库通过清晰的扩展清单、统一的构建脚本与 CI/CD 流水线，实现了可扩展、可维护、可安全发布的扩展体系。借助 product.json 与脚本，可将扩展发布至 Open VSX 市场；通过 GitHub Actions 实现自动化构建、签名与发布。

**最新优化**：通过移除@parcel/watcher依赖和实现智能依赖管理，显著提升了构建速度和包体积效率，消除了原生模块编译的复杂性。建议持续优化激活策略与包体积，确保用户体验与稳定性。
...
- **新增**：构建优化命令：
  - 清理依赖缓存：删除 node_modules 后重新安装
  - 强制重新安装：VSCODE_FORCE_INSTALL=1 npm install
  - 查看构建日志：检查 build/npm 脚本输出
