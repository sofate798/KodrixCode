...
/**
 * Configure Chromium/Electron command-line switches based on argv.json and CLI args.
 *
 * Applies supported switches in three categories:
 * - Electron switches: passed directly to Chromium via `app.commandLine.appendSwitch`
 * - Main process switches: pushed onto `process.argv` for downstream consumption
 * - Feature flags: platform-wide enable/disable/blink feature configuration
 *
 * @param cliArgs - Parsed CLI arguments from the command line
 * @returns The parsed argv.json configuration object
 */
...
/**
 * Read the argv.json configuration file synchronously before the app 'ready' event.
 *
 * Error handling strategy:
 * - ENOENT (file not found): create a default argv.json file silently
 * - JSON parse errors: log a warning with the specific syntax error for debugging
 * - Other IO errors: log a warning, fall back to empty defaults
 *
 * @returns Parsed argv.json contents, or an empty object on failure
 */
function readArgvConfigSync(): IArgvConfig {
	...
	try {
		...
		if (error?.code === 'ENOENT') {
			...
			// Distinguish JSON parse errors from other IO errors for easier debugging
			console.warn(
				`Unable to parse argv.json at '${argvConfigPath}': ${error.message}. ` +
				'Falling back to defaults. Please fix or delete the file.'
			);
		} else {
			console.warn(
				`Unable to read argv.json at '${argvConfigPath}', ` +
...
/**
 * Create a default argv.json file with helpful comments.
 *
 * Only called on first launch when argv.json does not exist.
 *
 * @param argvConfigPath - Absolute path where argv.json should be created
 */
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to Kodrix Code.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of Kodrix Code.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in Kodrix Code.',
...
/**
 * Resolve the path to argv.json.
 *
 * In portable mode, argv.json lives alongside the application directory.
 * Otherwise it resides in the user's data directory (e.g., `~/.vscode-oss-dev/argv.json`).
 */
...
/**
 * Configure the Electron crash reporter.
 *
 * Two modes are supported:
 * 1. **Local dump directory**: When `--crash-reporter-directory` is provided,
 *    crash reports are written to disk and NOT uploaded.
 * 2. **AppCenter upload**: Uses `product.json` platform-specific URLs with
 *    a crash reporter ID for authentication.
 */
...
/**
 * Collect JS flags from CLI args and argv.json.
 *
 * Flags are space-joined and passed to the V8 engine via `--js-flags`.
 *
 * @returns The joined flags string, or `null` if no flags are set
 */
...
/**
 * Parse command-line arguments using minimist with proper type declarations.
 *
 * **IMPORTANT**: All arguments used as `string` or `boolean` MUST be declared
 * in the corresponding arrays below. When minimist encounters a `--string-arg`
 * multiple times (e.g. `--locale en --locale zh-cn`), it returns `string[]` instead
 * of `string`. This function normalizes such arrays to the last value to match
 * the `NativeParsedArgs` type contract and prevent runtime crashes like
 * `locale.toLowerCase is not a function`.
 */
function parseCLIArgs(): NativeParsedArgs {
	...
	// Minimist returns string[] when a string argument is passed multiple times.
	// Normalize to the last value to match NativeParsedArgs types and avoid
	// runtime errors such as .toLowerCase() / .split() on arrays.
	...
	return parsed as NativeParsedArgs;
}

/**
 * Register early global event listeners on the Electron `app` object.
 *
 * Hooks `open-file` (macOS file drops before `app.ready`) and `open-url`
 * (deep links / protocol handlers) to capture these early events.
 * The captured data is exposed via `globalThis.getOpenUrls()` and
 * `globalThis.macOpenFiles` for consumption by the main process.
 */
...
/**
 * Determine the V8 code cache directory.
 *
 * The code cache speeds up subsequent launches by caching compiled
 * JavaScript bytecode. It is disabled when:
 * - `--no-cached-data` is present in process.argv
 * - Running in development mode (VSCODE_DEV)
 * - Product commit hash is missing (e.g., local build)
 *
 * @returns Absolute path to the code cache directory, or `undefined` if disabled
 */
...
/**
 * Normalize Chinese locales to our supported variants.
 *
 * On Windows/macOS, the system returns zh-hans / zh-hant tags.
 * On Linux, the system returns zh-CN / zh-TW / zh-SG / zh-MY country codes.
 *
 * Rules:
 * - `zh-hans`, `zh-CN`, `zh-SG`, `zh-MY` → Simplified Chinese (`zh-cn`)
 * - `zh-hant`, `zh-TW`, `zh-HK`, `zh-MO`, etc. → Traditional Chinese (`zh-tw`)
 * - `zh` without a region (plain "zh") → Simplified Chinese (`zh-cn`) by default
 *   as it is the most widely used variant of written Chinese.
 */
function processZhLocale(appLocale: string): string {
	...
	const region = appLocale.split('-')[1]?.toLowerCase();

	// Region is undefined when the locale is just "zh" without a region tag.
	// Default to Simplified Chinese in that case.
	if (!region) {
		return 'zh-cn';
	}
	...
	// Traditional Chinese regions: hant, tw, hk, mo, and any other zh-*
	return 'zh-tw';
}
...
function getUserDefinedLocale(argvConfig: IArgvConfig): string | undefined {
	// minimist may return string[] when --locale is passed multiple times, even
	// though NativeParsedArgs types it as string. Cast defensively.
	const locale: string | string[] | undefined = args['locale'] as string | string[] | undefined;
	if (typeof locale === 'string') {
		return locale.toLowerCase(); // a directly provided --locale always wins
	}
	if (Array.isArray(locale) && locale.length > 0) {
		const lastLocale = locale[locale.length - 1];
		if (typeof lastLocale === 'string') {
			return lastLocale.toLowerCase();
		}
	}
