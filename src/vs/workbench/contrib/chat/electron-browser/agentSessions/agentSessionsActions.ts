...
export class OpenWorkspaceInAgentsWindowAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('agentsWindow', "Agents Window"),
...
/**
 * Cursor-style purple pill shown in the Chat view header (auxiliary bar).
 */
class CursorAgentsWindowPillViewItem extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		container.classList.add('cursor-agents-window-pill');
		container.setAttribute('role', 'button');

		const hoverText = this.keybindingService.appendKeybinding(
			localize('agentsWindowHover', "Open Agents Window"),
		...
		const labelEl = append(container, $('span.cursor-agents-window-pill-label'));
		labelEl.textContent = this.action.label;

		const iconEl = append(container, $('span.cursor-agents-window-pill-icon'));
		reset(iconEl, renderIcon(Codicon.linkExternal));
...
export class OpenWorkspaceInAgentsContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		this._register(actionViewItemService.register(MenuId.ViewTitle, OPEN_WORKSPACE_IN_AGENTS_WINDOW_COMMAND_ID, (action, options) => {
			return instantiationService.createInstance(CursorAgentsWindowPillViewItem, action, options);
		}, undefined));
	}
}
...
export class AgentsHandoffInputTipContribution extends Disposable implements IWorkbenchContribution {
	 ...
	 * shows again the next time Kodrix Code is reopened.
	 */
