...
suite('chatRequiresSetup', () => {
	...
	test('not completed but anonymous access does not require setup', () => {
		assert.strictEqual(chatRequiresSetup(context({ completed: false, entitlement: ChatEntitlement.Unknown, anonymous: true })), false);
	});
