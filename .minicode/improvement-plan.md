# Minicode 体验提升方案 — 超越 Trae / Cursor / Qoder / Kiro / Windsurf
...
| **SOLO 四栏工作台** | 规划·对话·终端·预览，对标 Trae | ⭐⭐⭐⭐ |
...
| **Trae** | SOLO Builder 闭环、预览热更新 | SOLO 预览未做 HMR 热更新 |
...
**问题**：SOLO/Spec 工作台 Webview 是基础 HTML+CSS，缺少现代交互。

**改进方案**：

```
1. 骨架屏 (Skeleton Loading)
   - SOLO 工作台打开时，先渲染骨架屏而非空白
   - Spec 三栏加载时显示 pulsed placeholder
   - 实现：在 solo-workbench.html / spec-workbench.html 添加 skeleton CSS 动画
...
**实现位置**：
- `extensions/minicode-solo/resources/solo-workbench.html`
- `extensions/minicode-agent-os/resources/spec-workbench.html`
- `extensions/minicode-agent-os/resources/minicode-hub.html`

#### 2.2 Chat 面板体验升级

**问题**：Chat 面板还依赖原生 VS Code Chat UI，缺少 Minicode 独有的体验增强。
...
1. 智能输入框增强
   ...
   - @mention 自动补全改进：显示 Skill/Spec/SOLO 等自定义参与者
...
**问题**：目前通过 `minicode-local` 欢迎向导，但流程可更智能。
...
2. 导入增强
   ...
   - 冲突检测：Minicode 已有配置 vs 导入配置 → 让用户选择
...
1. 智能上下文建议
   ...
   - 在你输入 Chat 消息时，分析意图并建议附加上下文：
     "检测到你在问认证相关问题，要附加上 ./minicode/wiki/MODULES.md 吗？"
...
2. 工作流编排
   ...
   - 配置：.minicode/crew.json 定义 Agent 角色和工作流
...
3. 快捷键流
   - 打造类似 Vim 的操作流（但更现代）
   - Leader Key 模式：`Ctrl+;` → 进入 Minicode 模式 → 单键导航
   - `Ctrl+; s` = SOLO, `Ctrl+; p` = Plan, `Ctrl+; a` = Agent...
```
...
```
1. 共享 Event Bus
   在 minicode-agent-os 中创建 EventEmitter
   所有扩展通过它发布/订阅事件
   - minicode:wiki.updated
   - minicode:learning.new
   - minicode:agent.session.started / .completed
   - minicode:solo.build.progress
...
### 5.1 SOLO 工作台骨架屏
```css
/* 在 solo-workbench.html 的 <style> 中添加 */
.skeleton {
...
### 5.4 Chat 输入框提示增强
在 `cursor3Experience.ts` 或 `minicode-local` 中：
```typescript
...
Minicode 已经有非常扎实的基础——功能全面、架构清晰。当前的核心差距不在「功能数量」上，而在：
