<#
.SYNOPSIS
  Minicode Fast Development Launcher
...
.DESCRIPTION
  执行流程:
    ...
    [4/5] 编译 Minicode 自定义扩展 (minicode-local/skills/solo/agent-os)
    [5/5] 异步启动 Minicode.exe → 5 秒后释放控制台

.PARAMETER Watch
  启动后台 watch-transpile 进程 + Minicode
...
.EXAMPLE
  ...
  debug.bat -Prepare                  等同于 -NoLaunch

.NOTES
  内部被 debug.bat / watch-dev.bat 调用
...
function Set-BuiltinExtensionControl {
	$controlDir = Join-Path $env:USERPROFILE '.minicode-dev\extensions'
...
Write-Host '  Minicode Fast Debug' -ForegroundColor Magenta
Write-Host '=========================================' -ForegroundColor Magenta
Write-Host ''
...
if ($FullCompile -or -not (Test-Path $outDir)) {
	if ($FullCompile) {
	...
	Write-Step '[3/5] Client out/ exists — skipping compile (use -FullCompile to rebuild)'
}
...
if (-not (Test-MinicodeExtensionsCompiled)) {
	Write-Step '[4/5] Compiling Minicode extensions...'
	npm run gulp -- compile-extension:minicode-local compile-extension:minicode-skills compile-extension:minicode-solo compile-extension:minicode-agent-os
	if ($LASTEXITCODE -ne 0) { throw 'Minicode extension compile failed' }
} else {
	Write-Step '[4/5] Minicode extensions ready'
}
...
Write-Step '[5/5] Launching Minicode Dev...'
Write-Host ''
Write-Host '  Tip: edit src/ with -Watch for incremental rebuild' -ForegroundColor DarkGray
Write-Host '  Full rebuild: .\debug.bat -FullCompile' -ForegroundColor DarkGray
Write-Host ''
...
Write-Host "  Opening Minicode console window..." -ForegroundColor Green
...
Write-Host "  Minicode launching in separate console (launcher PID: $($proc.Id))." -ForegroundColor Green
Write-Host ""
...
if ($proc.HasExited -and $proc.ExitCode -ne 0) {
	...
	Write-Host "    - Electron executable not found (run debug.bat -Prepare first)" -ForegroundColor Yellow
	Write-Host "    - Node.js version mismatch (check .nvmrc)" -ForegroundColor Yellow
	Write-Host "    - Missing compile output (run debug.bat -FullCompile)" -ForegroundColor Yellow
	exit 1
}

Write-Host "  Minicode is now starting in a separate window." -ForegroundColor Green
Write-Host "  Close that window to stop Minicode, or close this one to exit." -ForegroundColor DarkGray
